<?php
session_start();
require_once __DIR__ . "/../../central_data/config/Database.php"; // added Database requirement
...
$pdo = new PDO("mysql:host=localhost;dbname=medical;charset=utf8", "root", "dealwiththeletters12");
...
if (isset($_GET['get_doctors']) && !empty($_GET['get_doctors'])) {
    $serviceName = $_GET['get_doctors'];

    $stmt = $conn->prepare("\n    SELECT D.DOC_ID, D.DOC_FIRST_NAME, D.DOC_LAST_NAME, SP.SPEC_NAME\n    FROM DOCTOR D\n    INNER JOIN SPECIALIZATION SP ON D.SPEC_ID = SP.SPEC_ID\n    WHERE D.SERV_ID = (\n        SELECT SERV_ID FROM SERVICE WHERE SERV_NAME = ?\n    )\n    ORDER BY D.DOC_LAST_NAME ASC\n    ");
    ...
    while ($row = $result->fetch_assoc()) {
        $doctors[] = [
            'id' => $row['DOC_ID'],
            'name' => "Dr. " . $row['DOC_FIRST_NAME'] . " " . $row['DOC_LAST_NAME'] . " - (" . $row['SPEC_NAME'] . ")"
        ];
    }
...
function get_or_create_service_name($conn, $serv_name) {
    $check = $conn->prepare("SELECT SERV_ID FROM SERVICE WHERE SERV_NAME = ?");
    $check->bind_param("s", $serv_name);
    $check->execute();
    $check->store_result();

    if ($check->num_rows === 0) {
        $check->close();
        $insert = $conn->prepare("INSERT INTO SERVICE (SERV_NAME) VALUES (?)");
        $insert->bind_param("s", $serv_name);
        $insert->execute();
        $insert->close();
    } else {
        $check->close();
...
function get_or_create_status_name($conn, $status_name) {
    $check = $conn->prepare("SELECT STATUS_ID FROM STATUS WHERE STATUS_NAME = ?");
    $check->bind_param("s", $status_name);
    $check->execute();
    $check->store_result();
...
if (isset($_POST['add_appt']) && in_array(strtolower($currentRole), ['superadmin', 'patient'])) {
    ...
    $stmt = $conn->prepare("SELECT SERV_ID FROM SERVICE WHERE SERV_NAME = ?");
    $stmt->bind_param("s", $serv_name);
    $stmt->execute();
    $stmt->bind_result($serv_id);
    if (!$stmt->fetch()) {
        $stmt->close();
        $insert = $conn->prepare("INSERT INTO SERVICE (SERV_NAME) VALUES (?)");
        $insert->bind_param("s", $serv_name);
    ...
    $stmt = $conn->prepare("INSERT INTO APPOINTMENT(APPT_ID, APPT_DATE, APPT_TIME, APPT_CREATED_AT, APPT_UPDATED_AT, PAT_ID, DOC_ID, SERV_ID, STAT_ID)\n        VALUES (?, ?, ?, NOW(), NOW(), ?, ?, ?, ?)");
...
if (isset($_POST['update_appt']) && in_array(strtolower($currentRole), ['superadmin', 'patient'])) {
    ...
    $stmt = $conn->prepare("UPDATE APPOINTMENT SET APPT_DATE = ?, APPT_TIME = ?, PAT_ID = ?, DOC_ID = ?, SERV_ID = ?, STAT_ID = ?, APPT_UPDATED_AT = NOW()\n        WHERE APPT_ID = ?\n    ");
    $stmt->bind_param("ssiiiis", $appt_date, $appt_time, $pat_id, $doc_id, $serv_id, $stat_id, $appt_id);
...
if ($search) {
    $like = "%$search%";
    $stmt = $conn->prepare("SELECT A.APPT_ID, A.APPT_DATE, A.APPT_TIME, 
    A.APPT_CREATED_AT, A.APPT_UPDATED_AT,A.PAT_ID, A.DOC_ID,S.SERV_NAME, ST.STATUS_NAME,
    P.PAT_FIRST_NAME, P.PAT_LAST_NAME, D.DOC_FIRST_NAME, D.DOC_LAST_NAME 
    FROM APPOINTMENT A 
...
<div id="addForm" class="add-form-container">
  <form method="POST" action="">
    ...
    <label>Service:</label>
<select name="serv_name" id="add_serv" required>
    <option value="">-- Select Service --</option>
    <?php
    $services = $conn->query("SELECT SERV_NAME FROM SERVICE ORDER BY SERV_NAME ASC");
    while ($s = $services->fetch_assoc()) {
        echo '<option value="' . htmlspecialchars($s['SERV_NAME']) . '">' . htmlspecialchars($s['SERV_NAME']) . '</option>';
    }
    ?>
</select>
...
<select name="doc_id" id="add_doc" required>
    ...
    <?php
    $doctors = $conn->query("
        SELECT D.DOC_ID, D.DOC_FIRST_NAME, D.DOC_MIDDLE_INIT, D.DOC_LAST_NAME, S.SPEC_NAME 
        FROM DOCTOR D
        INNER JOIN SPECIALIZATION S ON D.SPEC_ID = S.SPEC_ID
        ORDER BY D.DOC_LAST_NAME ASC
    ");
    while ($d = $doctors->fetch_assoc()) {
        $fullName = "Dr. " . $d['DOC_FIRST_NAME'] 
                    . (!empty($d['DOC_MIDDLE_INIT']) ? " " . $d['DOC_MIDDLE_INIT'] : "") 
                    . " " . $d['DOC_LAST_NAME'] 
                    . " - (" . $d['SPEC_NAME'] . ")";
        echo '<option value="' . $d['DOC_ID'] . '">' . htmlspecialchars($fullName) . '</option>';
    }
    ?>
</select>
...
<div id="updateForm" class="add-form-container">
  <form method="POST" action="">
    ...
    <label>Service:</label>
    <select name="serv_name" id="u_serv" required>
        <option value="">-- Select Service --</option>
        <?php
        $services = $conn->query("SELECT SERV_NAME FROM SERVICE ORDER BY SERV_NAME ASC");
        while ($s = $services->fetch_assoc()) {
...
function showUpdateForm(id, date, time, pat, doc, serv, status) {
    ...
    document.getElementById("u_serv").value = serv;
    document.getElementById("u_status").value = status;

    loadDoctors(serv, "u_doc", doc);
...
document.getElementById("add_serv").addEventListener("change", function () {
    ...
    if (service === "") {
        document.getElementById("add_doc").innerHTML =
            '<option value="">-- Select Doctor --</option>';
        return;
    }

    fetch("appointment.php?get_doctors=" + encodeURIComponent(service))
        .then(response => response.json())
        .then(data => {
            let docDropdown = document.getElementById("add_doc");
            docDropdown.innerHTML = '<option value="">-- Select Doctor --</option>';
...
document.addEventListener("change", function (e) {
    if (e.target.id === "upd_serv") {
        let service = e.target.value;

        if (service === "") {
            document.getElementById("upd_doc").innerHTML =
                '<option value="">-- Select Doctor --</option>';
            return;
        }
        ...
        fetch("appointment.php?get_doctors=" + encodeURIComponent(service))
            .then(response => response.json())
            .then(data => {
                let docDropdown = document.getElementById("upd_doc");
                docDropdown.innerHTML = '<option value="">-- Select Doctor --</option>';
...
function loadDoctors(serviceName, dropdownID, selectedDoctor = null) {
    if (!serviceName) return;

    fetch("?get_doctors=" + encodeURIComponent(serviceName))
        .then(response => response.json())
        .then(data => {
