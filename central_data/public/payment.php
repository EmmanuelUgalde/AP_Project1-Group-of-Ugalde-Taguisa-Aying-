...
$methods = $conn->query("SELECT PYMT_METH_ID, PYMT_METH_NAME FROM PAYMENT_METHOD")->fetch_all(MYSQLI_ASSOC);
$statuses = $conn->query("SELECT PYMT_STAT_ID, PYMT_STAT_NAME FROM PAYMENT_STATUS")->fetch_all(MYSQLI_ASSOC);
...
if(isset($_GET['success'])){
    if($_GET['success']=='added') $message="Payment added successfully!";
    if($_GET['success']=='updated') $message="Payment updated successfully!";
    if($_GET['success']=='deleted') $message="Payment deleted successfully!";
}
...
.container-page .search-bar button:hover {
    ...
    transition: background-color 0.3s ease;
        ...
        .btn-add { font-size: 19px; background-color: #28a745; color: white; width: 150px; height: 60px; margin-right: 15px;}
...
<table>
<tr>
<th>ID</th><th>Amount Paid</th><th>Date</th><th>Method</th><th>Status</th><th>Appointment</th><th>Actions</th>
</tr>
<?php if ($paymentList): ?>
...
<div id="addForm" class="add-form-container">
    <form method="POST">
        ...
        <label>Method:</label>
            <select name="pymt_meth_id" style="font-size: 20px;" required>
        ...
        <label>Status:</label>
            <select name="pymt_stat_id" style="font-size: 20px;" required>
        ...
        <label>Appointment:</label>
            <select name="appt_id" style="font-size: 20px;" required>
...
<div id="updateForm" class="add-form-container">
    <form method="POST">
        ...
        <label>Method:</label>
            <select name="pymt_meth_id" id="update_method" style="font-size: 20px;" required>
                <option value="">-- Select Method --</option>
                <?php foreach ($methods as $m): ?>
                <option value="<?= $m['PYMT_METH_ID'] ?>"><?= htmlspecialchars($m['PYMT_METH_NAME']) ?></option>
                <?php endforeach; ?>
        ...
        <label>Status:</label>
            <select name="pymt_stat_id" id="update_status" style="font-size: 20px;" required>
                <option value="">-- Select Status --</option>
                <?php foreach ($statuses as $s): ?>
                <option value="<?= $s['PYMT_STAT_ID'] ?>"><?= htmlspecialchars($s['PYMT_STAT_NAME']) ?></option>
                <?php endforeach; ?>
        ...
        <label>Appointment:</label>
            <select name="appt_id" id="update_appt" style="font-size: 20px;" required>
                <option value="">-- Select Appointment --</option>
                <?php foreach ($appts as $a): ?>
                <option value="<?= $a['APPT_ID'] ?>">#<?= $a['APPT_ID'] ?></option>
                <?php endforeach; ?>
...
function showUpdateForm(id, amount, date, methodId, statusId, apptId){document.getElementById('overlay').classList.add('show');const f=document.getElementById('updateForm');f.style.display='block';setTimeout(()=>f.classList.add('show'),10);document.getElementById('update_id').value=id;document.getElementById('update_amount').value=amount;document.getElementById('update_date').value=date;document.getElementById('update_method').value=methodId;document.getElementById('update_status').value=statusId;document.getElementById('update_appt').value=apptId;}
