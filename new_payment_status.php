...
$currentRole = strtolower($_SESSION['role']); // Keep it lowercase throughout
...
if (isset($_POST['add_status'])) {
    ...
    $check = $conn->prepare("SELECT COUNT(*) FROM PATIENT WHERE PAT_ID = ?");
    ...
    if ($count == 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO PAYMENT_STATUS (PAT_ID, PYMT_STAT_NAME) VALUES (?, ?)");
        $stmt->bind_param("is", $pat_id, $pymt_stat_name);
        if ($stmt->execute()) {
...
if (isset($_POST['update_status'])) {
    ...
    if ($count == 0) {
        ...
        $stmt = $conn->prepare("UPDATE PAYMENT_STATUS SET PAT_ID = ?, PYMT_STAT_NAME = ? WHERE PYMT_STAT_ID = ?");
        $stmt->bind_param("isi", $pat_id, $pymt_stat_name, $pymt_stat_id);
        if ($stmt->execute()) {
...
$search = $_GET['search'] ?? '';
if ($search) {
    $stmt = $conn->prepare("SELECT * FROM PAYMENT_STATUS WHERE PYMT_STAT_NAME LIKE ? OR PAT_ID LIKE ? ORDER BY PYMT_STAT_ID DESC");
    $like = "%$search%";
    $stmt->bind_param("ss", $like, $like);
} else {
    $stmt = $conn->prepare("SELECT * FROM PAYMENT_STATUS ORDER BY PYMT_STAT_ID DESC");
}
$stmt->execute();
...
<div class="container-page">
    ...
    <table>
        ...
        <?php if (!empty($statusList)): ?>
            <?php foreach ($statusList as $s): ?>
                <tr>
                    <td><?= htmlspecialchars($s['PYMT_STAT_ID']) ?></td>
                    <td><?= htmlspecialchars($s['PAT_ID'] ?? 'N/A') ?></td>
                    <td><?= htmlspecialchars($s['PYMT_STAT_NAME']) ?></td>
                    <td><?= htmlspecialchars($s['PYMT_STAT_CREATED_AT'] ?? 'N/A') ?></td>
                    <td><?= htmlspecialchars($s['PYMT_STAT_UPDATED_AT'] ?? 'N/A') ?></td>
                    <td>
                        <button type="button" class="btn btn-update" 
                            ...
                            onclick="showUpdateForm(
                                <?= (int)$s['PYMT_STAT_ID'] ?>,
                                <?= (int)($s['PAT_ID'] ?? 0) ?>,
                                '<?= htmlspecialchars($s['PYMT_STAT_NAME'], ENT_QUOTES) ?>'
                            )">
                            Edit
                        </button>
                        ...
                        <?php else: ?>
                            <button type="button" class="btn btn-delete" style="width:100px; height:40px; opacity: 0.5; cursor: not-allowed;" 
                                onclick="alert('Only SuperAdmin can delete payment statuses!');">
                                Delete
                            </button>
                        <?php endif; ?>
                    </td>
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <tr><td colspan="6">No payment status found.</td></tr>
        <?php endif; ?>
    </table>
    ...
    <div id="addForm" class="add-form-container">
        <form method="POST" action="">
            ...
            <input type="number" name="pat_id" required min="1" placeholder="Enter Patient ID">
    ...
    <div id="updateForm" class="add-form-container">
        <form method="POST" action="">
            ...
            <label>Patient ID:</label>
            <input type="number" name="pat_id" id="update_pat_id" required min="1" placeholder="Enter Patient ID">

            <label>Status Name:</label>
            <select id="update_name" name="pymt_stat_name" required>
                <option value="">-- Select Status --</option>
                <option value="Paid">Paid</option>
                <option value="Pending">Pending</option>
                <option value="Refunded">Refunded</option>
...
function showUpdateForm(id, patId, name) {
    ...
    document.getElementById('update_pat_id').value = patId;
